...
* @version $Id: kb_pages.php,v 1.7 2008/06/03 20:10:28 jonohlsson Exp $
...
class kb_pages
{
	var $target_file = 'kb.php';
	var $phpbb_block_map = array();
	var $cache_key = '_pagemap_kb';
	...
	{
		global $db, $mx_cache, $mx_root_path, $module_root_path, $phpEx, $mx_table_prefix;
		...
		$this->cache_key = '_pagemap_kb_' . $this->target_file;
		...
		if ( MXBB_27x )
		{
			...
			if ( !$this->page_id )
			{
				$this->page_id = get_page_id( 'kb.php', true );
			}
		...
		else
		{
			...
			else
			{
				...
				$sql = "SELECT col.page_id, blk.block_id, sys.parameter_value, fnc.function_file
						...
						AND par.parameter_type 	= 'kb_type_select'
