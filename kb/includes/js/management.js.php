    ...
    AjaxRequest.get(
	  {
	    ...
	    ,'parameters':{ 'mode':'article', 'k': escape(treeGetId().substr(treeGetId().lastIndexOf("/")+1)), 'id': escape(treeGetId()), 'block_id':mxBlock.block_id, 'page_id':mxBlock.page_id, 'start':mxBlock.start }
	    ,'onLoading':function() { clearTabs(); tabsLoadingOn(); }
	    ,'onComplete':function() { tabsLoadingOff(); }
...
function generateTOC(mode) {
	...
	tocdiv.innerHTML = '<div id="contents"></div>';
...
function generateParentTOC(mode) {
    ...
    if(!document.getElementById("sectioncontents")) {
    	...
    	document.getElementById("tabs-data").innerHTML = addTocDiv(document.getElementById("tabs-data").innerHTML);
    }
...
function addTocDiv(html) {
	html = '<div id="sectioncontents"></div>' + html;
	return html;
}
