 ...
 * -------------------------------------------------------------------------
 * 
 *    $Id: functions_comment.php,v 1.1 2005/12/08 15:06:46 jonohlsson Exp $
 */
...
class mx_kb_comments
{
	...
	{
		global $mx_kb, $kb_config, $db;
		
		$mx_kb->debug('comments->init (internal_comments='.$comments_type.')', basename( __FILE__ ));
		...
		//
		$this->forum_id = $mx_kb->modules[$mx_kb->module_name]->cat_rowset[$this->cat_id]['comments_forum_id'];
	
		$this->auth['auth_view'] = $mx_kb->modules[$mx_kb->module_name]->auth_user[$this->cat_id]['auth_comment'];
		$this->auth['auth_post'] = $mx_kb->modules[$mx_kb->module_name]->auth_user[$this->cat_id]['auth_comment'];
		$this->auth['auth_edit'] = $mx_kb->modules[$mx_kb->module_name]->auth_user[$this->cat_id]['auth_comment'];
		$this->auth['auth_delete'] = $mx_kb->modules[$mx_kb->module_name]->auth_user[$this->cat_id]['auth_comment'];
	...
	function display_comments()
	{
		...
		{
			case 'internal':
				$mx_kb->debug('comments->display_internal_comments', basename( __FILE__ ));
				$this->display_internal_comments();
			break;
			
			case 'phpbb':
				$mx_kb->debug('comments->display_phpbb_comments', basename( __FILE__ ));
	...
	{
		global $template, $mx_kb, $lang, $board_config, $phpEx, $kb_config, $db, $userdata, $images;
		...
		while ( $this->comments_row = $db->sql_fetchrow( $result ) )
		{
			...
			if (!$kb_config['allow_comment_images'] || !$kb_config['allow_comment_links'])
			{
				...
				$comments_text = $mx_kb_text_tools->remove_images_links( $comments_text, $kb_config['allow_comment_images'], $kb_config['no_comment_image_message'], $kb_config['allow_comment_links'], $kb_config['no_comment_link_message'] );
			}
						
			if ( $kb_config['allow_bbcode'] )
			{
				if ( $comments_text != '' && $this->comments_row['comment_bbcode_uid'] != '' )
				{
					$comments_text = ( $kb_config['allow_bbcode'] ) ? bbencode_second_pass( $comments_text, $this->comments_row['comment_bbcode_uid'] ) : preg_replace( '/\:[0-9a-z\:]+\]/si', ']', $comments_text );
				}
			}
			...
			// Text formatting
			//
			if ( $kb_config['max_comment_subject_chars'] > 0 )
			{
				$comments_title = $mx_kb_text_tools->truncate_text( $comments_title, $kb_config['max_comment_subject_chars'], true );
			}
	
			if ( $kb_config['max_comment_chars'] > 0 )
			{
				$comments_text = $mx_kb_text_tools->truncate_text( $comments_text, $kb_config['max_comment_chars'], true );
			}		
			
			if ( $kb_config['formatting_comment_truncate_links'] || $kb_config['formatting_comment_image_resize'] > 0 || $kb_config['formatting_comment_wordwrap'] ) 
			{
				$comments_text = $mx_kb_text_tools->decode( $comments_text, $kb_config['formatting_comment_truncate_links'], intval($kb_config['formatting_comment_image_resize']), $kb_config['formatting_comment_wordwrap'] );
			}
			...
			if ( ( $this->auth['auth_edit'] && $this->comments_row['user_id'] == $userdata['user_id'] ) || $this->auth['auth_mod'] )
			{
				$template->assign_block_vars( 'use_comments.text.auth_edit', array(
					...
					'U_COMMENT_EDIT' => append_sid( this_kb_mxurl( 'mode=post_comment&item_id=' . $this->item_id . '&cat_id=' . $this->cat_id . '&cid='.$this->comments_row['comments_id'] ) ),
			...
			if ( ( $this->auth['auth_delete'] && $this->comments_row['user_id'] == $userdata['user_id'] ) || $this->auth['auth_mod'] )
			{
				$template->assign_block_vars( 'use_comments.text.auth_delete', array(
					...
					'U_COMMENT_DELETE' => append_sid( this_kb_mxurl( "mode=post_comment&cid=".$this->comments_row['comments_id']."&delete=do&item_id=".$this->item_id . '&cat_id=' . $this->cat_id )),
		...
		if ( ( $this->auth['auth_post'] ) || $this->auth['auth_mod'] )
		{
			$template->assign_block_vars( 'use_comments.auth_post', array(
				...
				'U_COMMENT_POST' => append_sid( this_kb_mxurl( 'mode=post_comment&item_id=' . $this->item_id . '&cat_id=' . $this->cat_id) ),
				'REPLY_IMG' => $phpbb_root_path . $images['icon_reply'],
			));
		}

		$num_of_replies = intval( $this->total_comments );
		$pagination = generate_pagination( this_kb_mxurl( $this->pagination_action . "&" . $this->pagination_target . $this->item_id . $page_num ), $num_of_replies, $this->pagination_num, $this->start ) . '&nbsp;';
		if ($num_of_replies > 0)
		{
	...
	function display_phpbb_comments( )
	{
		...
		while ( $this->comments_row = $db->sql_fetchrow( $result ) )
		{
			...
			if ( ( $this->auth['auth_edit'] && $this->comments_row['user_id'] == $userdata['user_id'] ) || $this->auth['auth_mod'] )
			{
				$template->assign_block_vars( 'use_comments.text.auth_edit', array(
					'L_COMMENT_EDIT' => $lang['Comment_edit'],
					'U_COMMENT_EDIT' => append_sid( this_kb_mxurl( 'mode=post_comment&item_id=' . $this->item_id . '&cat_id=' . $this->cat_id . '&cid='.$this->comments_row['post_id'] ) ),
					'EDIT_IMG' => $phpbb_root_path . $images['icon_edit'], 
				));
			}
			...
			if ( ( $this->auth['auth_delete'] && $this->comments_row['user_id'] == $userdata['user_id'] ) || $this->auth['auth_mod'] )
			{
				$template->assign_block_vars( 'use_comments.text.auth_delete', array(
					'L_COMMENT_DELETE' => $lang['Comment_delete'],
					'U_COMMENT_DELETE' => append_sid( this_kb_mxurl( "mode=post_comment&cid=".$this->comments_row['post_id']."&delete=do&item_id=".$this->item_id . '&cat_id=' . $this->cat_id )),
					'DELETE_IMG' => $phpbb_root_path . $images['icon_delpost'], 
				));
			}
		...
		if ( ( $this->auth['auth_post'] ) || $this->auth['auth_mod'] )
		{
			$template->assign_block_vars( 'use_comments.auth_post', array(
				'L_COMMENT_ADD' => $lang['Comment_add'],
				'U_COMMENT_POST' => append_sid( this_kb_mxurl( 'mode=post_comment&item_id=' . $this->item_id . '&cat_id=' . $this->cat_id ) ),
				'REPLY_IMG' => $images['kb_comment_post'],
			));
		}
	...
	function get_internal_comments()
	{
		...
		$sql = "SELECT COUNT(article_id) AS number
		...
		$sql = 'SELECT c.*, u.*
			...
			WHERE c.article_id = '" . $this->item_id . "'
