...
* @version $Id: kb_mcp.php,v 1.5 2008/06/03 20:10:33 jonohlsson Exp $
 ...
 *
 */
class mx_kb_mcp extends mx_kb_public
{
	/**
	 * Enter description here...
	 *
	...
	function main( $action )
	{
		...
		global $mx_kb_functions, $template, $kb_config;
		...
		else if ( $do_mode == 'do_delete' )
		{
			if ( is_array( $ids ) && !empty( $ids ) )
			{
				foreach( $ids as $temp_id )
				{
					...
					// Comments
					//
					if ($this->comments[$item_info['article_category_id']]['activated'] && $kb_config['del_topic'])
					{
						...
						else
						{
							if ( $item_info['topic_id'] )
							{
								...
								$mx_kb_comments->init( $item_info, 'phpbb');
								$mx_kb_comments->post('delete_all', $item_info['topic_id']);
							}
						}
					}
		...
		$template->assign_vars( array(
			'ARTICLES' => $kb_config['module_name'],
			'U_ARTICLES' => mx_append_sid( $this->this_mxurl() ),
			'L_MCP_TITLE' => $l_title,
			'L_MCP_EXPLAIN' => $l_explain,

			'S_HIDDEN_FIELDS' => $s_hidden_fields,
			'S_ACTION' => mx_append_sid( $this->this_mxurl( "mode=mcp" ) )
		));
		...
		if ( in_array( $mode, array( 'unapproved', 'cat', 'all' ) ) )
		{
			...
			$template->assign_vars( array(
				...
				'PAGINATION' => phpBB2::generate_pagination( mx_append_sid( $this->this_mxurl( "mode=mcp&amp;mode_mcp=$mode&amp;sort_method=$sort_method&amp;sort_order=$sort_order" ) . ($mode == 'cat' ? "&amp;cat_id=$cat_id" : '') ), $total_num, $kb_config['pagination'], $start ),
				'PAGE_NUMBER' => sprintf( $lang['Page_of'], ( floor( $start / $kb_config['pagination'] ) + 1 ), ceil( $total_num / $kb_config['pagination'] ) ),
				...
				if ( isset( $data['row_set'] ) )
				{
					...
					foreach( $data['row_set'] as $item_data )
					{
						...
						$template->assign_block_vars( 'mcp_mode.row', array(
							...
							'U_EDIT' => mx_append_sid( $this->this_mxurl( "mode=edit&amp;k={$item_data['article_id']}" ) ),
							'U_DELETE' => mx_append_sid( $this->this_mxurl( "mode=mcp&amp;mode_mcp=$mode&amp;do_mode=do_delete&amp;id={$item_data['article_id']}" ) ),
							'U_APPROVE' => mx_append_sid( $this->this_mxurl( "mode=mcp&amp;mode_mcp=$mode&amp;do_mode=$approve_mode&amp;id={$item_data['article_id']}" ) . ($mode == 'cat' ? "&amp;cat_id=$cat_id" : '') ),
