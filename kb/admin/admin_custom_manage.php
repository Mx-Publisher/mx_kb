...
* @version $Id: admin_custom_manage.php,v 1.8 2008/07/15 22:05:42 jonohlsson Exp $
...
class mx_kb_custom_manage extends mx_kb_admin
{
	function main( $action )
	{
		...
		$this_custom_field = new mx_custom_field(KB_CUSTOM_TABLE, KB_CUSTOM_DATA_TABLE);
		$this_custom_field->init();
		...
		$field_type = ( isset( $_REQUEST['field_type'] ) ) ? intval( $_REQUEST['field_type'] ) : $this_custom_field->field_rowset[$field_id]['field_type'];
		...
		if ( $submit )
		{
			...
			{
				$this_custom_field->update_add_field( $field_type );

				$message = $lang['Fieldadded'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( 'admin_kb.' . $phpEx . '?action=custom_manage' ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( 'index.' . $phpEx . '?pane=right' ) . '">', '</a>' );
				mx_message_die( GENERAL_MESSAGE, $message );
			}
			elseif ( $mode == 'do_add' && $field_id )
			{
				$this_custom_field->update_add_field( $field_type, $field_id );

				$message = $lang['Fieldedited'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( 'admin_kb.' . $phpEx . '?action=custom_manage' ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( 'index.' . $phpEx . '?pane=right' ) . '">', '</a>' );
				mx_message_die( GENERAL_MESSAGE, $message );
			}
			elseif ( $mode == 'do_delete' )
			{
				...
				{
					$this_custom_field->delete_field( $key );
				}

				$message = $lang['Fieldsdel'] . '<br /><br />' . sprintf( $lang['Click_return'], '<a href="' . mx_append_sid( 'admin_kb.' . $phpEx . '?action=custom_manage' ) . '">', '</a>' ) . '<br /><br />' . sprintf( $lang['Click_return_admin_index'], '<a href="' . mx_append_sid( 'index.' . $phpEx . '?pane=right' ) . '">', '</a>' );
				mx_message_die( GENERAL_MESSAGE, $message );
			}
		}
		...
		$template->assign_vars( array(
			...
			'S_FIELD_ACTION' => mx_append_sid( "admin_kb.$phpEx?action=custom_manage" )
		));
		...
		if ( $mode == 'addfield' || $mode == 'editfield')
		{
			if ( $field_id )
			{
				$data = $this_custom_field->get_field_data( $field_id );
			}
		...
		{
			foreach( $this_custom_field->field_rowset as $field_id => $field_data )
			{
